...
@Component({
  name: "process-modal-edit",
  components: { BaseModal, BaseTitle, BaseButton, BaseLabel, BaseInput, BaseSelect }
})
export default class ProcessModalEdit extends Vue {
  @Prop(Number) pProcessId!: number;
  @Prop(String) pProcessName!: string;
  @Prop(String) pProcessArgs!: string;
  @Prop() pProcessExecId!: any;

  private processId: number = this.pProcessId;
  // プロセス名
  private processName: string = this.pProcessName;
  // 引数
  private processArgs: string = this.pProcessArgs;
  // 実行ID
  private processExecId: any = this.pProcessExecId;
  // 実行一覧
  ...
  public clickRegist() {
    ...
    process._id = this.processId;
    process._name = this.processName;
    process._args = this.processArgs;
    process._exec_id = this.processExecId;
    return process;
  }
}
</script>
